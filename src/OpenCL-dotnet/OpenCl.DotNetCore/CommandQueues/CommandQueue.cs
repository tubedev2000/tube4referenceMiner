...
namespace OpenCl.DotNetCore.CommandQueues
{
    ...
    public class CommandQueue : HandleBase
    {
        ...
        {
            IntPtr waitEventPointer = IntPtr.Zero;
            IntPtr edgesPtr = Marshal.AllocHGlobal(8 * 40);

            try
            {
                Marshal.Copy(edges, 0, edgesPtr, 40);

                Result result = EnqueuedCommandsNativeApi.EnqueueWriteBuffer(this.Handle, memoryObject.Handle, 1, new UIntPtr((uint)0), new UIntPtr((uint)40*8), edgesPtr, 0, null, waitEventPointer);
